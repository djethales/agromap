...
<header class="relative w-full">
    <div class="relative w-full overflow-hidden
            @if($post->title == 'foresteries & environnement')
                bg-agri-forest
            @endif
            @if($post->title == 'agriculture durable')
                bg-agri-dura
            @endif
            @if($post->title == 'digitalisation agricole')
                bg-agri-digit
            @endif
            @if($post->title == 'géomatique')
                bg-agri-geoma
            @endif
    " style="height: 500px;">
    ...
    <section class=" pt-24">
        <div class="px-6   lg:container lg:mx-auto lg:px-8 ">
            <div class=" flex flex-col items-center">
                <div class=" md:w-8/12 lg:w-10/12 ">
